...
\begin{itemize}
    \item \textbf{~~输入层：}对于CBOW模型来说，输入层包含某一个单词$w_i$的上下文$2c$个单词的词向量$v \left( \text {context} ( w ) _ { 1 } \right) , v \left( \text {context} ( w ) _ { 2 } \right) , \ldots , \quad v \left( \text {context} ( w ) _ { 2 c } \right) \in \mathbb { R } ^ { m }$，对于skip-gram模型来说，输入层的输入就是一个单词的向量$v(w)$。这里$m$是词向量的维度(长度)。
...
\subsection{基于图建模的可解释性推荐系统}

\subsection{基于知识库的可解释性推荐系统}
